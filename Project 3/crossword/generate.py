...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for variable in self.domains:
            ...
            for val in self.domains[variable]:
                if len(val) != var_length:
                    remove.add(val)
            
            self.domains[variable] -= remove
            
    def overlap(self, x, y, value_x, value_y):
        'overlap method '
    ...
    def revise(self, x, y):
        ...
        for value_x in set(self.domains[x]):
            ...
            for value_y in self.domains[y]:
                if value_x != value_y and self.overlap(x,y,value_x, value_y):
                    consistent = True
                    break
            if not consistent:
                remove.add(value_x)
    ...
    def consistent(self, assignment):
        ...
        for x in assignment:
            ...
            if len(x_val) != x.length:
                return False
            
            for y in self.crossword.neighbors(x):
                if y in assignment:
                    y_val = assignment[y]
                    if not self.overlap(x, y, x_val, y_val):
                        return False
        return True
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        ruleout = {vals: 0 for vals in self.domains[var]}
        for val in self.domains[var]:
            for other_var in self.crossword.neighbors(var):
                for other_val in self.domains[other_var]:
                    if not self.overlap(var, other_var, val, other_val):
                        ruleout[val] += 1
        
        return sorted([x for x in ruleout], key=lambda x: ruleout[x])
