...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                #if it is otherwise, add them to the sentance if they are in the board 
                if 0 <= i < self.height and 0 <= j < self.width:
                    new_sentence_cells.add((i, j))
                
        # add the new senance to the knowledge base of the AI 
        self.knowledge.append(Sentence(new_sentence_cells, count))
        knowledge_changed = True
        # go through iteratively and mark the guaranteed mines and safes 
        while knowledge_changed:
            ...
            for sentance in self.knowledge:
                safes = safes.union(sentance.known_safes())
                mines = mines.union(sentance.known_mines()) 
        ...
        for sentence_1 in self.knowledge:
            for sentence_2 in self.knowledge:
                ...
                if sentence_1.cells == set() and sentence_1.count > 0:
                    raise ValueError("No cells and count created")
    ...
    def make_random_move(self):
        ...
        basic_probability = num_mines_left / spaces_left
        
        for i in range(self.height):
            for j in range(self.width):
                if (i, j) not in self.moves_made and (i, j) not in self.mines:
                    moves[(i, j)] = basic_probability
        ...
        if moves and not self.knowledge:
            ...
            for sentence in self.knowledge:
                ...
                mine_probability = sentence.count / num_cells
            ...
            best_moves = [x[0] for x in move_list if x[1] == best_probability]
